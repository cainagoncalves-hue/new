-- ============================================================
-- MIGRATION 026: get_iso_scores(p_mes) — filtro por mês de referência
--
-- Parâmetro p_mes: string "YYYY-MM" (ex: "2026-04")
--   - ''  → comportamento anterior: dados mais recentes globalmente
--   - não vazio → calcula o ISO para aquele mês específico:
--       • Turnover e CID-F: apenas registros daquele mês
--       • LNPS: pesquisa mais recente cujo month_key ≤ p_mes
--       • ISBE: idem, mais recente até p_mes por gestor/time
-- ============================================================

DROP FUNCTION IF EXISTS get_iso_scores();
DROP FUNCTION IF EXISTS get_iso_scores(text);

CREATE OR REPLACE FUNCTION get_iso_scores(p_mes text DEFAULT '')
RETURNS TABLE (
  gestor_nome        text,
  area               text,
  headcount          bigint,
...
-- ── LNPS — pesquisa mais recente (até p_mes, se fornecido) ──────────────────
...
best_lnps AS (
  ...
  WHERE month_key <> ''
    AND (p_mes = '' OR month_key <= p_mes)   -- ≤ p_mes quando filtrado
...
-- ── ISBE — mês mais recente por gestor (até p_mes, se fornecido) ─────────────
...
-- ── ISBE por time — fallback (até p_mes, se fornecido) ──────────────────────
...
-- ── Turnover voluntário — filtrado pelo mês exato ────────────────────────────
turnover_cte AS (
  SELECT nome_gestor AS gestor, COUNT(*) AS n
  FROM manual_desligamentos
  WHERE tipo = 'voluntario'
    AND nome_gestor IS NOT NULL AND nome_gestor <> ''
    AND (p_mes = '' OR DATE_TRUNC('month', mes_referencia) = (p_mes || '-01')::date)
  GROUP BY nome_gestor
),

-- ── CID F — filtrado pelo mês exato ──────────────────────────────────────────
cidf_cte AS (
...
scores AS (
  SELECT
    ...
    -- LNPS direto; fallback = média dos times do líder
    ...
    -- ISBE direto; fallback = média dos times do líder
    ...
    -- Metadados das pesquisas (período efetivamente usado)
