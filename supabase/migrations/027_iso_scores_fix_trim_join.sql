-- ============================================================
-- MIGRATION 027: Corrige mismatch de TRIM nos JOINs de get_iso_scores
--
-- A migration 026 adicionou TRIM(nome_gestor) no CTE `leaders`,
-- mas os CTEs de LNPS e ISBE retornam nome_gestor sem TRIM (vindos das
-- tabelas de survey). Isso quebrava os JOINs para líderes cujo nome
-- tem espaço extra no elofy_users — o ISO retornava NULL para eles.
--
-- Correção: restaura leaders/leader_teams sem TRIM (como na 022) e
-- aplica TRIM nos dois lados dos JOINs dentro do CTE `scores`.
-- Os filtros de p_mes da migração 026 são preservados integralmente.
-- ============================================================
...
-- Survey mais recente ≤ p_mes (quando informado)
best_lnps AS (
...
-- ── Turnover voluntário ───────────────────────────────────────────────────────
-- Quando p_mes fornecido, filtra pelo mês exato de referência
turnover_cte AS (
...
-- JOINs usam TRIM() nos dois lados para tolerar espaços extras vindos
-- da API Elofy — sem alterar os GROUP BY dos CTEs anteriores.
scores AS (
  SELECT
    l.gestor,
    l.area,
