-- ============================================================
-- MIGRATION 037: LNPS do ISO passa a considerar o ciclo mais
-- recente POR GESTOR/TIME, em vez de uma única pesquisa "mais
-- recente" global.
--
-- Bug: a CTE `best_lnps` escolhia UMA pesquisa LNPS (a de maior
-- month_key) para a empresa toda. Quando um ciclo era direcionado
-- a um único time (ex: pré-vendas), ele virava "a mais recente"
-- para todo mundo, e todo líder fora daquele público ficava sem
-- nenhuma resposta em `lnps_resp` — o COALESCE(s.lnps_score, 0)
-- no cálculo final então zerava 40% do ISO desses líderes, mesmo
-- eles tendo dados legítimos de ciclos anteriores.
--
-- Correção: aplica o mesmo padrão já usado para ISBE (ver
-- isbe_latest_per_gestor / isbe_team_latest na 027) — cada gestor
-- (e, no fallback multi-time, cada time) usa o próprio ciclo LNPS
-- mais recente disponível, em vez de depender de um pick global.
--
-- Nenhuma outra parte da função (ISBE, turnover, CID F, score
-- final) muda nesta migration.
-- ============================================================
