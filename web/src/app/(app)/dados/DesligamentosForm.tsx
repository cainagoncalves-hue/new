...
import { addDesligamento, deleteDesligamento } from "./actions";
...
interface Props {
  gestores: string[];
  records: Desligamento[];
}

const MONTH = new Date().toISOString().slice(0, 7);

export default function DesligamentosForm({ gestores, records }: Props) {
  const formRef = useRef<HTMLFormElement>(null);
  const [err, action, pending] = useActionState(async (_: unknown, fd: FormData) => {
    try {
      await addDesligamento(fd);
    ...
    <div>
      <form ref={formRef} action={action} style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 24 }}>
        ...
        <div style={{ ...fieldStyle, gridColumn: "1 / -1" }}>
          ...
          <textarea name="justificativa" rows={2} style={{ ...inputStyle, resize: "vertical" }} placeholder="Opcional" />
        </div>
        {err && <div style={{ gridColumn: "1 / -1", color: "var(--red)", fontSize: 12 }}>{err}</div>}
        <div style={{ gridColumn: "1 / -1" }}>
          <button type="submit" disabled={pending} style={btnStyle}>
            {pending ? "Salvando..." : "Adicionar Desligamento"}
          </button>
        </div>
      </form>
      ...
      {records.length > 0 && (
        <div style={{ overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
            <thead>
              <tr>
                {["Colaborador", "Gestor", "Mês", "Tipo", "Justificativa", ""].map(h => (
                ...
                <tr key={r.id} style={{ borderBottom: "1px solid var(--border)" }}>
                  ...
                  <td style={tdStyle}>
                    <span style={{
                      ...
                      fontSize: 11,
                      fontWeight: 600,
                      background: r.tipo === "voluntario" ? "var(--amber-pale, #fef3c7)" : "var(--surface-2)",
                      color: r.tipo === "voluntario" ? "#92400e" : "var(--text-500)",
                    }}>
                      {r.tipo}
                    </span>
                  </td>
                  ...
                  <td style={tdStyle}>
                    <form action={deleteDesligamento.bind(null, r.id)}>
